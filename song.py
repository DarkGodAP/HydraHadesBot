...
class MusicCog(commands.Cog):
    ...
    async def ensure_voice(self, ctx: Union[commands.Context, discord.Interaction]) -> Optional[discord.VoiceClient]:
        ...
        if not channel_attr or not channel_attr.channel:
            if isinstance(ctx, discord.Interaction):
                await ctx.followup.send("❌ You must be connected to a voice channel.", ephemeral=True)
            else:
    ...
    async def add_and_play(self, ctx_or_inter: Union[commands.Context, discord.Interaction], query: str):
        ...
        if not info:
            text = "❌ Could not extract any information for that query."
            if is_inter:
        ...
        if not stream_url:
            txt = ("❌ No playable stream URL found. YouTube may be forcing SABR-only streaming. "
        ...
        if is_inter:
            await ctx_or_inter.followup.send(f"✅ Queued **{track['title']}** (position {pos}).", ephemeral=True)
        else:
            await ctx_or_inter.send(f"✅ Queued **{track['title']}** (position {pos}).")
    ...
    async def spotify_playlist(self, ctx_or_inter: Union[commands.Context, discord.Interaction], playlist_id_or_url: str):
        ...
        if not pid:
            msg = ("❌ Could not parse a Spotify playlist id from your input. "
        ...
        try:
            ...
            txt = f"❌ Could not fetch playlist: {e}"
            if is_inter:
                await ctx_or_inter.followup.send(txt, ephemeral=True)
            else:
                await ctx_or_inter.send(txt)
        ...
        if is_inter:
            await ctx_or_inter.followup.send(f"✅ Added {added} tracks from Spotify playlist to the queue. ({player.get('playlist_name') or 'unknown name'})", ephemeral=False)
        else:
            await ctx_or_inter.send(f"✅ Added {added} tracks from Spotify playlist to the queue. ({player.get('playlist_name') or 'unknown name'})")
    ...
    async def _play_next(self, guild_id: int):
        ...
        if not next_track.get("stream_url") and next_track.get("query"):
            ...
            if not resolved:
                ...
                if guild:
                    ...
                    if chan:
                        await chan.send(f"⚠️ Skipped track (could not resolve): {next_track.get('title')}")
        ...
        try:
            ...
            if guild:
                chan = next((c for c in guild.text_channels if c.permissions_for(guild.me).send_messages), None)
                if chan:
                    await chan.send(f"❌ Playback error: {e}")
